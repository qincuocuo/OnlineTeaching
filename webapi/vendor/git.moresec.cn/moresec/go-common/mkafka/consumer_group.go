...
func OptionConsumerGroupSasl(user string, passwd string) ConsumerGroupOption {
	return func(cg *ConsumerGroup) error {
		cg.config.Net.SASL.Enable = true
		cg.config.Net.SASL.Version = 1
		cg.config.Net.SASL.User = user
		cg.config.Net.SASL.Password = passwd
		return nil
	}
}
