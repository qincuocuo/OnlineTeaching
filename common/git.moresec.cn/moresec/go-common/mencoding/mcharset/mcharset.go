...
func Convert(dstCharset string, srcCharset string, src string) (dst string, err error) {
	...
	if srcCharset != "UTF-8" {
		if e := getEncoding(srcCharset); e != nil {
			...
			if err != nil {
				return "", merror.WrapCodef(mcode.CodeInternalError, err, "%s to utf8 failed", srcCharset)
			}
			src = string(tmp)
		} else {
			return dst, merror.NewCodef(mcode.CodeInvalidParameter, "unsupported srcCharset: %s", srcCharset)
		}
	}
	...
	if dstCharset != "UTF-8" {
		if e := getEncoding(dstCharset); e != nil {
			...
			if err != nil {
				return "", merror.WrapCodef(mcode.CodeInternalError, err, "utf to %s failed", dstCharset)
			}
			dst = string(tmp)
		} else {
			return dst, merror.NewCodef(mcode.CodeInvalidParameter, "unsupported dstCharset: %s", dstCharset)
		}
	} else {
		dst = src
	}
...
func getEncoding(charset string) encoding.Encoding {
	...
	enc, _ := ianaindex.MIB.Encoding(charset)
	return enc
}
