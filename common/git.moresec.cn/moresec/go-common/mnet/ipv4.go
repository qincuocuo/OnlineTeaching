...
import (
	...
	"git.moresec.cn/moresec/go-common/mtext/mregex"
)
...
func Validate(ip string) bool {
	return mregex.IsMatchString(`^((25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(25[0-5]|2[0-4]\d|[01]?\d\d?)$`, ip)
}

// ParseAddress parses `address` to its ip and port.
// Eg: 192.168.1.1:80 -> 192.168.1.1, 80
func ParseAddress(address string) (string, int) {
	match, err := mregex.MatchString(`^(.+):(\d+)$`, address)
	if err == nil {
