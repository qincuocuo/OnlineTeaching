...
func Scan(params interface{}, pointer interface{}, mapping ...map[string]string) (err error) {
	...
	if pointerType == nil {
		return merror.NewCode(mcode.CodeInvalidParameter, "parameter pointer should not be nil")
	}
...
func doScanList(structSlice interface{}, structSlicePointer interface{}, bindToAttrName, relationAttrName, relationFields string) (err error) {
	...
	if bindToAttrName == "" {
		return merror.NewCode(mcode.CodeInvalidParameter, `bindToAttrName should not be empty`)
	}
	...
	for i := 0; i < arrayValue.Len(); i++ {
		...
		if len(relationDataMap) > 0 && !relationFromAttrValue.IsValid() {
			return merror.NewCodef(mcode.CodeInvalidParameter, `invalid relation fields specified: "%v"`, relationFields)
		}
		...
		if relationFields != "" && !relationBindToFieldNameChecked {
			...
			if !relationFromAttrField.IsValid() {
				var (
					filedMap, _ = mstructs.FieldMap(mstructs.FieldMapInput{
						Pointer:         relationFromAttrValue,
						RecursiveOption: mstructs.RecursiveOptionEmbeddedNoTag,
					})
				)
		...
		default:
			return merror.NewCodef(mcode.CodeInvalidParameter, `unsupported attribute type: %s`, bindToAttrKind.String())
		}
	}
