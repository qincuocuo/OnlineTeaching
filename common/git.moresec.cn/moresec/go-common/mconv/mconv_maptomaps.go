...
func doMapToMaps(params interface{}, pointer interface{}, mapping ...map[string]string) (err error) {
	...
	if paramsKind != reflect.Array && paramsKind != reflect.Slice {
		return merror.NewCode(mcode.CodeInvalidParameter, "params should be type of slice, eg: []map/[]*map/[]struct/[]*struct")
	}
	var (
	...
	if paramsElemKind != reflect.Map && paramsElemKind != reflect.Struct && paramsElemKind != reflect.Interface {
		return merror.NewCodef(mcode.CodeInvalidParameter, "params element should be type of map/*map/struct/*struct, but got: %s", paramsElemKind)
	}
	...
	if pointerKind != reflect.Array && pointerKind != reflect.Slice {
		return merror.NewCode(mcode.CodeInvalidParameter, "pointer should be type of *[]map/*[]*map")
	}
	var (
	...
	if pointerElemKind != reflect.Map {
		return merror.NewCode(mcode.CodeInvalidParameter, "pointer element should be type of map/*map")
	}
	defer func() {
