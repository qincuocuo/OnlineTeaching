...
import (
	...
	"git.moresec.cn/moresec/go-common/internal/empty"
	"git.moresec.cn/moresec/go-common/internal/json"
	"git.moresec.cn/moresec/go-common/internal/utils"
	"git.moresec.cn/moresec/go-common/merrors/mcode"
	"git.moresec.cn/moresec/go-common/merrors/merror"
	"git.moresec.cn/moresec/go-common/mos/mstructs"
)
...
func doStruct(params interface{}, pointer interface{}, mapping map[string]string, priorityTag string) (err error) {
	...
	if pointer == nil {
		return merror.NewCode(mcode.CodeInvalidParameter, "object pointer cannot be nil")
	}

	defer func() {
		// Catch the panic, especially the reflection operation panics.
		if exception := recover(); exception != nil {
			if v, ok := exception.(error); ok && merror.HasStack(v) {
				err = v
			} else {
				err = merror.NewCodeSkipf(mcode.CodeInternalError, 1, "%+v", exception)
			}
		}
	}()
	...
	if v, ok := pointer.(reflect.Value); ok {
		...
		if pointerReflectKind != reflect.Ptr {
			return merror.NewCodef(mcode.CodeInvalidParameter, "object pointer should be type of '*struct', but got '%v'", pointerReflectKind)
		}
	...
	if paramsMap == nil {
		return merror.NewCodef(
			mcode.CodeInvalidParameter,
	...
	tagToNameMap, err := mstructs.TagMapName(pointerElemReflectValue, priorityTagArray)
	if err != nil {
		return err
	}
	...
	defer func() {
		if exception := recover(); exception != nil {
			if err = bindVarToReflectValue(structFieldValue, value, mapping); err != nil {
				err = merror.WrapCodef(mcode.CodeInternalError, err, `error binding value to attribute "%s"`, name)
			}
		}
	}()
	// Directly converting.
