...
import (
	...
	"git.moresec.cn/moresec/go-common/internal/rwmutex"
	"git.moresec.cn/moresec/go-common/mconv"
	"git.moresec.cn/moresec/go-common/mtext/mstr"
)
...
func NewFrom(items interface{}, safe ...bool) *Set {
	...
	for _, v := range mconv.Interfaces(items) {
		m[v] = struct{}{}
	}
	return &Set{
		data: m,
...
func (set *Set) Join(glue string) string {
	...
	for k := range set.data {
		buffer.WriteString(mconv.String(k))
		if i != l-1 {
...
func (set *Set) String() string {
	...
	for k := range set.data {
		...
		if mstr.IsNumeric(s) {
			...
			buffer.WriteString(`"` + mstr.QuoteMeta(s, `"\`) + `"`)
		}
		if i != l-1 {
			buffer.WriteByte(',')
		}
...
func (set *Set) UnmarshalValue(value interface{}) (err error) {
	...
	switch value.(type) {
	case string, []byte:
		err = json.UnmarshalUseNumber(mconv.Bytes(value), &array)
	default:
		array = mconv.SliceAny(value)
	}
	for _, v := range array {
		set.data[v] = struct{}{}
	}
